...
ControlCommand DroneController::update(
		tum_ardrone::filter_stateConstPtr state) {
	ptamIsGood = state->ptamState == state->PTAM_BEST
			|| state->ptamState == state->PTAM_GOOD
			|| state->ptamState == state->PTAM_TOOKKF;
	scaleAccuracy = state->scaleAccuracy;

	// calculate (new) errors.
	TooN::Vector<4> new_err = TooN::makeVector(target.pos[0] - state->x,
			target.pos[1] - state->y, target.pos[2] - state->z,
			target.yaw - state->yaw);

	// speeds
	TooN::Vector<4> speeds = TooN::makeVector(state->dx, state->dy, state->dz,
			state->dyaw);
...
void DroneController::calcControl(TooN::Vector<4> new_err,
		TooN::Vector<4> d_error, double yaw) {
	...
	for (int i = 0; i < 4; i++)
		if (targetNew[i] > 0.5 && getMS() / 1000.0 - targetSetAtClock > 0.1
				&& last_err[i] * new_err[i] < 0) {
			i_term[i] = 0;
			targetNew[i] = 0;
		}
	...
	lastSentControl.yaw = std::min(max_yaw,
			std::max(-max_yaw, (double) (lastSentControl.yaw * agr)));
	...
	// clip
	lastSentControl.roll = std::min(max_rp,
			std::max(-max_rp, (double) (lastSentControl.roll * agr)));
	lastSentControl.pitch = std::min(max_rp,
			std::max(-max_rp, (double) (lastSentControl.pitch * agr)));
	...
	lastSentControl.gaz = std::min(max_gaz_rise / rise_fac,
			std::max(max_gaz_drop, gazP + gazD + gazI));
	if (lastSentControl.gaz > 0)
		lastSentControl.gaz *= rise_fac;

	logInfo = TooN::makeVector(Kp_rp * p_term[0], Kp_rp * p_term[1], gazP,
			Kp_yaw * p_term[3], Kd_rp * d_term[0], Kd_rp * d_term[1], gazD,
			Kd_yaw * d_term[3], Ki_rp * i_term[0], Ki_rp * i_term[1], gazI,
			Ki_yaw * i_term[3], lastSentControl.roll, lastSentControl.pitch,
			lastSentControl.gaz, lastSentControl.yaw, lastSentControl.roll,
			lastSentControl.pitch, lastSentControl.gaz, lastSentControl.yaw,
			new_err[0], new_err[1], new_err[2], new_err[3], target.pos[0],
			target.pos[1], target.pos[2], target.yaw);
}
