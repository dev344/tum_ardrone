...
inline static void rod2rpy(TooN::SO3<> trans, double* roll, double* pitch,
		double* yaw) {
...
inline static int getMS(ros::Time stamp = ros::Time::now()) {
	if (ros_header_timestamp_base == 0) {
		...
		std::cout << "set ts base to " << ros_header_timestamp_base
				<< std::endl;
	}
	int mss = (stamp.sec - ros_header_timestamp_base) * 1000
			+ stamp.nsec / 1000000;
...
// [ziquan]
inline static double smallestAngleToTarget(double angle, double targetAngle) {
	while (angle < targetAngle - 180)
		angle += 360;
	while (angle >= targetAngle + 180)
		angle -= 360;
	return angle;
...
inline static double angleToValidYaw(double angle) {
	return smallestAngleToTarget(angle, 0);
}
