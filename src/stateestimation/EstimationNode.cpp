...
EstimationNode::EstimationNode()
{
    ...
    navdata_sub = nh_.subscribe(navdata_channel, 10, &EstimationNode::navdataCb,
            this);
    ...
    dronepose_pub = nh_.advertise < tum_ardrone::filter_state
            > (output_channel, 1);

    compare_pub = nh_.advertise < tum_ardrone::compare_state
            > (compare_channel, 1); //[ziquan]
    tum_ardrone_pub = nh_.advertise < std_msgs::String > (command_channel, 50);
    tum_ardrone_sub = nh_.subscribe(command_channel, 50, &EstimationNode::comCb,
            this);
        ...
        const ardrone_autonomy::NavdataConstPtr navdataPtr)
{
    ...
    // save last timestamp
    if (lastNavStamp != ros::Time(0)
            && (lastNavdataReceived.header.stamp - lastNavStamp
                    > ros::Duration(0.1)))
    ...
    if (logfileIMU != NULL)
    {
        ...
        if (logfileIMU != NULL)
            (*logfileIMU) << getMS(lastNavdataReceived.header.stamp) << " "
                    << lastNavdataReceived.tm << " " << lastNavdataReceived.vx
                    << " " << lastNavdataReceived.vy << " "
                    << lastNavdataReceived.altd << " "
                    << lastNavdataReceived.rotX / 1000.0 << " "
                    << lastNavdataReceived.rotY / 1000.0 << " "
                    << lastNavdataReceived.rotZ / 1000.0 << " "
                    << lastNavdataReceived.pressure << " " << 0 << " " << 0
...
void EstimationNode::Loop()
{
    ...
    while (nh_.ok())
    {
        ...
        pthread_mutex_lock(&filter->filter_CS);
        tum_ardrone::filter_state s = filter->getPoseAt(
                ros::Time().now() + predTime);
        pthread_mutex_unlock(&filter->filter_CS);
        ...
        uint32_t level)
{
    ...
    ptamWrapper->setPTAMPars(config.PTAMMinKFTimeDiff,
            config.PTAMMinKFWiggleDist, config.PTAMMinKFDist);
...
void EstimationNode::publishTf(TooN::SE3<> trans, ros::Time stamp, int seq,
        std::string system)
{
    trans = trans.inverse();
    ...
    if (logfilePTAMRaw != NULL)
    {
        ...
        if (logfilePTAMRaw != NULL)
            (*(logfilePTAMRaw)) << seq << " " << stamp << " "
                    << tr.getOrigin().x() << " " << tr.getOrigin().y() << " "
                    << tr.getOrigin().z() << " " << tr.getRotation().x() << " "
                    << tr.getRotation().y() << " " << tr.getRotation().z()
...
void EstimationNode::toogleLogging()
{
    ...
    if (stat((packagePath + std::string("/logs")).c_str(), &st) != 0)
        mkdir((packagePath + std::string("/logs")).c_str(),
                S_IXGRP | S_IXOTH | S_IXUSR | S_IRWXU | S_IRWXG | S_IROTH);
    ...
    if (logfileIMU == 0)
    {
        ...
        sprintf(buf, "u l ENABLED LOGGING to %s/logs/%ld", packagePath.c_str(),
                currentLogID);
        publishCommand(buf);
    }
    else
    ...
    if (logfileIMU == 0)
    {
        ...
        sprintf(buf, "%s/logs/%ld/logIMU.txt", packagePath.c_str(),
                currentLogID);
        logfileIMU->open(buf);
    }
    else
    ...
    if (logfilePTAM == 0)
    {
        ...
        sprintf(buf, "%s/logs/%ld/logPTAM.txt", packagePath.c_str(),
                currentLogID);
    ...
    if (logfileFilter == 0)
    {
        ...
        sprintf(buf, "%s/logs/%ld/logFilter.txt", packagePath.c_str(),
                currentLogID);
    ...
    if (logfilePTAMRaw == 0)
    {
        ...
        sprintf(buf, "%s/logs/%ld/logPTAMRaw.txt", packagePath.c_str(),
                currentLogID);
    ...
    if (quitLogging)
    {
        ...
        char buf2[200];
        sprintf(buf, "%s/logs/%ld", packagePath.c_str(), currentLogID);
        sprintf(buf2, "%s/logs/%ld-%lds", packagePath.c_str(), currentLogID,
                (getMS() - startedLogClock + 500) / 1000);
        rename(buf, buf2);
    }
...
void EstimationNode::reSendInfo()
{
    ...
    if (pos != std::string::npos)
        found = sscanf(ptamMsg.substr(pos).c_str(),
                "Found: %d/%d %d/%d %d/%d %d/%d Map: %dP, %dKF", &kpf[0],
                &kps[0], &kpf[1], &kps[1], &kpf[2], &kps[2], &kpf[3], &kps[3],
                &kp, &kf);
    char bufp[200];
    if (found == 10)
        snprintf(bufp, 200, "Map: KF: %d, KP: %d (%d of %d found)", kf, kp,
                kpf[0] + kpf[1] + kpf[2] + kpf[3],
    ...
    snprintf(buf, 1000,
            ...
            ptamStatus.c_str(), bufp, filter->getCurrentScales()[0],
            filter->scalePairsIn, filter->scalePairsOut,
            filter->getScaleAccuracy(),
            filter->useScalingFixpoint ? "FIX" : "DRONE", status.c_str(),
            (int) lastNavdataReceived.batteryPercent);
