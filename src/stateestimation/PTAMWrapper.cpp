...
void PTAMWrapper::HandleFrame()
{
    ...
    // }
    std::cout << "z value " << PTAMPoseGuess[1] << " " << PTAMPoseGuess[2]
            << std::endl;
            ...
            && (forceKF || mpMap->vpKeyFrames.size() < maxKF || maxKF <= 1))
    {
        ...
        if (mpTracker->lastStepResult == Tracker::T_TOOK_KF)
        {
            ...
            std::cerr << filterPosePrePTAM.slice<0, 6>() << std::endl
                    << std::endl;
    ...
    { // Testing
        if (gTracking < 5)
        {
            ...
            // Create a rectangle mask
            {
                ...
                int half_height = 120;
                cv::rectangle(mask,
                        cv::Point(center_x - half_width,
                                center_y - half_height),
                        cv::Point(center_x + half_width,
                                center_y + half_height), cv::Scalar(255), -1, 8,
                        0);
            }
