...
void PTAMWrapper::ResetInternal()
{
    ...
    {
        std::cout << "Waiting for first navdata to determine drone version!"
                << std::endl;
        usleep(250000);
    }
    if (file.size() == 0)
    ...
    fleH.close();
    std::cout << "Set Camera Paramerer to: " << camPar[0] << " " << camPar[1]
            << " " << camPar[2] << " " << camPar[3] << " " << camPar[4]
            << std::endl;

    mpMap = new Map;
    mpCamera = new ATANCamera(camPar);
    mpMapMaker = new MapMaker(*mpMap, *mpCamera);
    mpTracker = new Tracker(CVD::ImageRef(frameWidth, frameHeight), *mpCamera,
            *mpMap, *mpMapMaker);
...
void PTAMWrapper::setPTAMPars(double minKFTimeDist, double minKFWiggleDist,
        double minKFDist)
{
    if (mpMapMaker != 0)
...
void PTAMWrapper::run()
{
    ...
    snprintf(charBuf, 200, "Video resolution: %d x %d", frameWidth,
            frameHeight);
    ROS_INFO(charBuf);
    node->publishCommand(std::string("u l ") + charBuf);

    // create window
    myGLWindow = new GLWindow2(CVD::ImageRef(frameWidth, frameHeight),
            "PTAM Drone Camera Feed", this);
...
//			<< filterPosePrePTAM[5] << std::endl;
    ...
    TooN::Vector<6> PTAMPoseGuess = filter->backTransformPTAMObservation(
    ...
    node->publishTf(mpTracker->GetCurrentPose(), mimFrameTimeRos_workingCopy,
            mimFrameSEQ_workingCopy, "cam_front");

    // 1. multiply from left by frontToDroneNT.
    // 2. convert to xyz,rpy
    predConvert->setPosSE3_globalToDrone(
            predConvert->frontToDroneNT * PTAMResultSE3);
    TooN::Vector<6> PTAMResult = TooN::makeVector(predConvert->x,
            predConvert->y, predConvert->z, predConvert->roll,
            predConvert->pitch, predConvert->yaw);
...
//			<< PTAMResultTransformed[5] << std::endl;
    ...
    TooN::Vector<6> diffs = PTAMResultTransformed
            - filterPosePrePTAM.slice<0, 6>();
    for (int i = 0; 1 < 1; i++)
        diffs[i] = abs(diffs[i]);
...
//			<< filterPosePostPTAM[5] << std::endl << std::endl;
    ...
    if (isGoodCount >= 3)
    {
        ...
        if (includedTime >= 2000 && framesIncludedForScaleXYZ > 1)
        { // ADD! (if too many, was reset before...)
            ...
            TooN::Vector<3> diffIMU = evalNavQue(
                    ptamPositionForScaleTakenTimestamp - filter->delayVideo
                            + filter->delayXYZ,
                    mimFrameTime_workingCopy - filter->delayVideo
                            + filter->delayXYZ, &zCorrupted, &allCorrupted,
                    &pressureStart, &pressureEnd);

            pthread_mutex_lock(&logScalePairs_CS);
            if (logfileScalePairs != 0)
                (*logfileScalePairs) << pressureStart << " " << pressureEnd
                        << " " << diffIMU[2] << " " << diffPTAM[2] << std::endl;
        ...
        if (framesIncludedForScaleXYZ == -1)
        { // RESET!
    ...
    if (lockNextFrame && isGood)
    {
        ...
        snprintf(charBuf, 500, "locking scale fixpoint to %.3f %.3f %.3f",
                PTAMResultTransformed[0], PTAMResultTransformed[1],
                PTAMResultTransformed[2]);
        ROS_INFO(charBuf);
        node->publishCommand(std::string("u l ") + charBuf);
    }

    // ----------------------------- Take KF? -----------------------------------
    if (!mapLocked && isGood //isVeryGood [ziquan]
            && (forceKF || mpMap->vpKeyFrames.size() < maxKF || maxKF <= 1))
    {
    ...
    if (!mapLocked && rand() % 5 == 0)
    {
        ...
        {
            predConvert->setPosSE3_globalToDrone(
                    predConvert->frontToDroneNT
                            * mpMap->vpKeyFrames[i]->se3CfromW);
            TooN::Vector<6> CamPos = TooN::makeVector(predConvert->x,
                    predConvert->y, predConvert->z, predConvert->roll,
                    predConvert->pitch, predConvert->yaw);
            CamPos = filter->transformPTAMObservation(CamPos);
            predConvert->setPosRPY(CamPos[0], CamPos[1], CamPos[2], CamPos[3],
                    CamPos[4], CamPos[5]);
    ...
    snprintf(charBuf + 20, 800,
            "scale: %.3f (acc: %.3f)                            ",
            filter->getCurrentScales()[0], (double) filter->getScaleAccuracy());
    snprintf(charBuf + 50, 800, "PTAM time: %i ms                            ",
            (int) (1000 * timeALL.toSec()));
    snprintf(charBuf + 68, 800, "(%i ms total)  ",
            (int) (1000 * timeALL.toSec()));
    if (mapLocked)
    ...
    if (mpMap->IsGood())
    {
        ...
        {
            snprintf(charBuf, 1000, "\nPTAM Diffs:              ");
            snprintf(charBuf + 13, 800, "x: %.3f                          ",
                    diffs[0]);
            snprintf(charBuf + 23, 800, "y: %.3f                          ",
                    diffs[1]);
            snprintf(charBuf + 33, 800, "z: %.3f                          ",
                    diffs[2]);
            snprintf(charBuf + 43, 800, "r: %.2f                          ",
                    diffs[3]);
            snprintf(charBuf + 53, 800, "p: %.2f                          ",
                    diffs[4]);
            ...
            snprintf(charBuf, 1000, "\nPTAM Pose:              ");
            snprintf(charBuf + 13, 800, "x: %.3f                          ",
                    PTAMResultTransformed[0]);
            snprintf(charBuf + 23, 800, "y: %.3f                          ",
                    PTAMResultTransformed[1]);
            snprintf(charBuf + 33, 800, "z: %.3f                          ",
                    PTAMResultTransformed[2]);
            snprintf(charBuf + 43, 800, "r: %.2f                          ",
                    PTAMResultTransformed[3]);
            snprintf(charBuf + 53, 800, "p: %.2f                          ",
                    PTAMResultTransformed[4]);
            ...
            snprintf(charBuf, 1000, "\nPTAM WiggleDist:              ");
            snprintf(charBuf + 18, 800, "%.3f                          ",
                    mpMapMaker->lastWiggleDist);
            snprintf(charBuf + 24, 800, "MetricDist: %.3f",
                    mpMapMaker->lastMetricDist);
            msg += charBuf;
        }
    }
    ...
    {
        // render grid
        predConvert->setPosRPY(filterPosePostPTAM[0], filterPosePostPTAM[1],
                filterPosePostPTAM[2], filterPosePostPTAM[3],
                filterPosePostPTAM[4], filterPosePostPTAM[5]);
    ...
    if (node->logfilePTAM != NULL)
    {
        ...
        if (node->logfilePTAM != NULL)
            (*(node->logfilePTAM)) << (isGood ? (isVeryGood ? 2 : 1) : 0) << " "
                    << (mimFrameTime_workingCopy - filter->delayVideo) << " "
                    << filterPosePrePTAM[0] << " " << filterPosePrePTAM[1]
                    << " " << filterPosePrePTAM[2] << " "
                    << filterPosePrePTAM[3] << " " << filterPosePrePTAM[4]
                    << " " << filterPosePrePTAM[5] << " "
                    << filterPosePrePTAM[6] << " " << filterPosePrePTAM[7]
                    << " " << filterPosePrePTAM[8] << " "
                    << filterPosePrePTAM[9] << " " << filterPosePostPTAM[0]
                    << " " << filterPosePostPTAM[1] << " "
                    << filterPosePostPTAM[2] << " " << filterPosePostPTAM[3]
                    << " " << filterPosePostPTAM[4] << " "
                    << filterPosePostPTAM[5] << " " << filterPosePostPTAM[6]
                    << " " << filterPosePostPTAM[7] << " "
                    << filterPosePostPTAM[8] << " " << filterPosePostPTAM[9]
                    << " " << PTAMResultTransformed[0] << " "
                    << PTAMResultTransformed[1] << " "
                    << PTAMResultTransformed[2] << " "
                    << PTAMResultTransformed[3] << " "
                    << PTAMResultTransformed[4] << " "
                    << PTAMResultTransformed[5] << " " << scales[0] << " "
                    << scales[1] << " " << scales[2] << " " << offsets[0] << " "
                    << offsets[1] << " " << offsets[2] << " " << offsets[3]
                    << " " << offsets[4] << " " << offsets[5] << " " << sums[0]
                    << " " << sums[1] << " " << sums[2] << " " << PTAMResult[0]
                    << " " << PTAMResult[1] << " " << PTAMResult[2] << " "
                    << PTAMResult[3] << " " << PTAMResult[4] << " "
                    << PTAMResult[5] << " " << PTAMResultSE3TwistOrg[0] << " "
                    << PTAMResultSE3TwistOrg[1] << " "
                    << PTAMResultSE3TwistOrg[2] << " "
                    << PTAMResultSE3TwistOrg[3] << " "
                    << PTAMResultSE3TwistOrg[4] << " "
                    << PTAMResultSE3TwistOrg[5] << " " << videoFramePing << " "
                    << mimFrameTimeRos_workingCopy << " "
                    << mimFrameSEQ_workingCopy << std::endl;
...
void PTAMWrapper::renderGrid(TooN::SE3<> camFromWorld)
{
    ...
    for (int i = 0; i < nTot; i++)
        for (int j = 0; j < nTot; j++)
        {
            ...
            //v3Cam[2] *= 100;
            if (v3Cam[2] < 0.001)
                v3Cam = TooN::makeVector(100000 * v3Cam[0], 100000 * v3Cam[1],
                        0.0001);
...
TooN::Vector<3> PTAMWrapper::evalNavQue(unsigned int from, unsigned int to,
        bool* zCorrupted, bool* allCorrupted, float* out_start_pressure,
        float* out_end_pressure)
{
    ...
    for (std::deque<ardrone_autonomy::Navdata>::iterator cur =
            navInfoQueue.begin(); cur != navInfoQueue.end();)
    {
        ...
        {
            if (curStampMs >= (int) from - pressureAverageRange
                    && curStampMs <= (int) from + pressureAverageRange)
            {
    ...
    *allCorrupted = abs(firstAdded - (int) from) + abs(lastAdded - (int) to)
            > 80;
    ...
    *out_start_pressure = sum_first / num_first;
    ...
    return TooN::makeVector(predIMUOnlyForScale->x, predIMUOnlyForScale->y,
            predIMUOnlyForScale->z);
}
...
void PTAMWrapper::newNavdata(ardrone_autonomy::Navdata* nav)
{
    ...
    if (lastNavinfoReceived.header.seq > 2000000
            || lastNavinfoReceived.header.seq < 0)
    {
