...
{
    Vector<2> v2Projected = project(
            m3Homography * unproject(match.v2CamPlaneFirst));
...
bool HomographyInit::Compute(vector<HomographyMatch> vMatches,
        double dMaxPixelError, SE3<> &se3SecondFromFirst)
{
...
void HomographyInit::RefineHomographyWithInliers()
{
    ...
    for (unsigned int i = 0; i < mvHomographyInliers.size(); i++)
    {
        ...
        m29Jacobian[0].slice<6, 3>() = -unproject(v2First) * dNumerator
                / (dDenominator * dDenominator);
        // For y:
        m29Jacobian[1].slice<0, 3>() = Zeros;
        m29Jacobian[1].slice<3, 3>() = unproject(v2First) / dDenominator;
        ;
        dNumerator = v3Second[1];
        m29Jacobian[1].slice<6, 3>() = -unproject(v2First) * dNumerator
                / (dDenominator * dDenominator);

        vmJacobians.push_back(
                mvHomographyInliers[i].m2PixelProjectionJac * m29Jacobian);
    }
...
void HomographyInit::DecomposeHomography()
{
    ...
    for (int signs = 0; signs < 4; signs++)
    {
        ...
        double dSinTheta = (d1 - d3) * x1_PM * x3_PM * e1[signs] * e3[signs]
                / d2;
    ...
    {
        mvDecompositions[i].se3SecondFromFirst.get_rotation() = s * U
                * mvDecompositions[i].m3Rp * V.T();
        mvDecompositions[i].se3SecondFromFirst.get_translation() = U
                * mvDecompositions[i].v3Tp;
    }
}
...
static double SampsonusError(Vector<2> &v2Dash, const Matrix<3> &m3Essential,
        Vector<2> &v2)
{
...
void HomographyInit::ChooseBestDecomposition()
{
    ...
    for (unsigned int i = 0; i < mvDecompositions.size(); i++)
    {
        ...
        for (unsigned int m = 0; m < mvHomographyInliers.size(); m++)
        {
            ...
            double dVisibilityTest = (mm3BestHomography[2][0] * v2[0]
                    + mm3BestHomography[2][1] * v2[1] + mm3BestHomography[2][2])
                    / decom.d;
    ...
    double dRatio = (double) mvDecompositions[1].nScore
    ...
    else // two-way ambiguity. Resolve by sampsonus score of all points.
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
                m3Essential.T()[j] = se3.get_translation()
                        ^ se3.get_rotation().get_matrix().T()[j];

            double dSumError = 0;
            for (unsigned int m = 0; m < mvMatches.size(); m++)
            {
                double d = SampsonusError(mvMatches[m].v2CamPlaneSecond,
                        m3Essential, mvMatches[m].v2CamPlaneFirst);
