...
ATANCamera::ATANCamera(TooN::Vector<5> params)
{
    ...
    GV2.Register(mgvvCameraParams, "Camera.Parameters", params,
            HIDDEN | SILENT);
...
void ATANCamera::RefreshParams()
{
    ...
    // (This only really makes sense for square-ish pixels)
    {
        ...
        Vector<2> v2RootTwoAway = UnProject(
                mvImageSize / 2 + vec(ImageRef(1, 1)));
...
Vector<2> ATANCamera::UFBProject(const Vector<2>& vCam)
{
    ...
    mvLastIm[0] = (*mgvvCameraParams)[2]
            + (*mgvvCameraParams)[0] * mvLastDistCam[0];
    mvLastIm[1] = (*mgvvCameraParams)[3]
            + (*mgvvCameraParams)[1] * mvLastDistCam[1];
    return mvLastIm;
}
...
{
    mvLastIm = v2Im;
    mvLastDistCam[0] = (mvLastIm[0] - (*mgvvCameraParams)[2])
            / (*mgvvCameraParams)[0];
    mvLastDistCam[1] = (mvLastIm[1] - (*mgvvCameraParams)[3])
            / (*mgvvCameraParams)[1];
    mdLastDistR = sqrt((double) (mvLastDistCam * mvLastDistCam));
    mdLastR = invrtrans(mdLastDistR);
    double dFactor;
...
const Vector<NUMTRACKERCAMPARAMETERS> ATANCamera::mvDefaultParams = makeVector(
        0.5, 0.75, 0.5, 0.5, 0.1);
