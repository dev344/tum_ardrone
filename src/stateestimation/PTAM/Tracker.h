...
{
public:
    Tracker(CVD::ImageRef irVideoSize, const ATANCamera &c, Map &m,
            MapMaker &mm);
    ...
    inline void setPredictedCamFromW(SE3<>& camFromW)
    {
...
protected:
    ...
    Vector<6> CalcPoseUpdate(std::vector<TrackerData*> vTD,
            double dOverrideSigma = 0.0, bool bMarkOutliers = false); // Updates pose from found points.
