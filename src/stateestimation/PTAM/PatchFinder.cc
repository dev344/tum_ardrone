...
int PatchFinder::CalcSearchLevelAndWarpMatrix(MapPoint &p, SE3<> se3CFromW,
        Matrix<2> &m2CamDerivs)
{
    ...
    mm2WarpInverse.T()[0] = m2CamDerivs
            * (v3MotionRight.slice<0, 2>()
                    - v3Cam.slice<0, 2>() * v3MotionRight[2] * dOneOverCameraZ)
            * dOneOverCameraZ;
    mm2WarpInverse.T()[1] = m2CamDerivs
            * (v3MotionDown.slice<0, 2>()
    ...
    Vector<3> result2 = (p.pPatchSourceKF)->se3CfromW.get_rotation()
            * unit_vector;
...
void PatchFinder::MakeTemplateCoarse(MapPoint &p, SE3<> se3CFromW,
        Matrix<2> &m2CamDerivs)
{
...
void PatchFinder::MakeTemplateCoarseNoWarp(KeyFrame &k, int nLevel,
        ImageRef irLevelPos)
{
...
bool PatchFinder::FindPatchCoarse(ImageRef irPos, KeyFrame &kf,
        unsigned int nRange)
{
    mbFound = false;
...
double PatchFinder::IterateSubPix(KeyFrame &kf)
{
    ...
    double dPixelUpdateSquared = v3Update.slice<0, 2>()
            * v3Update.slice<0, 2>();
