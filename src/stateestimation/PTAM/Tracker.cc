...
int gTransformsGood = 0;

// The constructor mostly sets up interal reference variables
// to the other classes..
Tracker::Tracker(ImageRef irVideoSize, const ATANCamera &c, Map &m,
        MapMaker &mm) :
        mMap(m), mMapMaker(mm), mCamera(c), mRelocaliser(mMap, mCamera), mirSize(
                irVideoSize)
{
    ...
    Reset();
    mappoints_pub = n.advertise < std_msgs::String > ("ptam/mappoints", 5000);
}
...
void Tracker::TrackFrame(Image<byte> &imFrame, bool bDraw)
{
    ...
    static gvar3<double> gvdSBIBlur("Tracker.RotationEstimatorBlur",
            TRACKER_ROTATION_ESTIMATOR_BLUR, SILENT);
    ...
    if (mbDraw)
    {
        ...
        if (GV2.GetInt("Tracker.DrawFASTCorners",
                TRACKER_DRAW_FAST_CORNERS_DEFAULT, SILENT))
        {
            glColor3f(1, 0, 1);
            glPointSize(1);
            glBegin(GL_POINTS);
            for (unsigned int i = 0; i < mCurrentKF.aLevels[0].vCorners.size();
                    i++)
    ...
    if (mMap.IsGood())
    {
        ...
        if (mTrackingQuality == GOOD
                && (force
                        || (mMapMaker.NeedNewKeyFrame(mCurrentKF)
                                && ((double) (clock()
                                        - mnLastKeyFrameDroppedClock))
                                        / CLOCKS_PER_SEC > minKFTimeDist
                                && mMapMaker.QueueSize() < 3)))
        {
            mMessageForUser << " Adding key-frame.";
            AddNewKeyFrame();
...
void Tracker::TrackForInitialMap()
{
    ...
    static gvar3<int> gvnMaxSSD("Tracker.MiniPatchMaxSSD",
            TRACKER_MINIPATCH_MAX_SSD_DEFAULT, SILENT);
    ...
    if (mnInitialStage == TRAIL_TRACKING_NOT_STARTED)
    {
        if (mbUserPressedSpacebar) // First spacebar = this is the first keyframe
        {
            ...
            lastStepResult = I_FIRST;
            KFZeroDesiredCamFromWorld = predictedCFromW;
        }
        else
        {
            lastStepResult = NOT_TRACKING;
            mMessageForUser
    ...
    if (mnInitialStage == TRAIL_TRACKING_STARTED)
    {
        ...
        if (mbUserPressedSpacebar)
        {
            ...
            for (list<Trail>::iterator i = mlTrails.begin();
                    i != mlTrails.end(); i++)
                vMatches.push_back(
                        pair<ImageRef, ImageRef>(i->irInitialPos,
                                i->irCurrentPos));
            bool succ = mMapMaker.InitFromStereo(mFirstKF, mCurrentKF, vMatches,
                    mse3CamFromWorld, KFZeroDesiredCamFromWorld,
                    predictedCFromW); // This will take some time!
            if (succ)
...
void Tracker::TrailTracking_Start()
{
    ...
    { // so that we can choose the image corners with max ST score
        ...
        if (!mCurrentKF.aLevels[0].im.in_image_with_border(c.irLevelPos,
                MiniPatch::mnHalfPatchSize))
            continue;
        vCornersAndSTScores.push_back(
                pair<double, ImageRef>(-1.0 * c.dSTScore, c.irLevelPos)); // negative so highest score first in sorted list
    };
...
int Tracker::TrailTracking_Advance()
{
    ...
    for (list<Trail>::iterator i = mlTrails.begin(); i != mlTrails.end();)
    {
        ...
        if (bFound)
        {
            ...
            bFound = BackwardsPatch.FindPatch(irBackWardsFound,
                    lPreviousFrame.im, 10, lPreviousFrame.vCorners);
...
void Tracker::TrackMap()
{
    ...
    for (unsigned int i = 0; i < mMap.vpPoints.size(); i++)
    {
        ...
        //
        // [Devesh] This would probably be a good place to prune out points
        // which are facing more than 70 degress away from the camera.
        // I also wonder why the PTAM guys did not do this (??)
    ...
    static gvar3<unsigned int> gvnCoarseMin("Tracker.CoarseMin",
            TRACKER_COARSE_MIN_DEFAULT, SILENT); // Min number of large-scale features for coarse stage
    static gvar3<unsigned int> gvnCoarseMax("Tracker.CoarseMax",
            TRACKER_COARSE_MAX_DEFAULT, SILENT); // Max number of large-scale features for coarse stage
    static gvar3<unsigned int> gvnCoarseRange("Tracker.CoarseRange",
            TRACKER_COARSE_RANGE_DEFAULT, SILENT); // Pixel search radius for coarse features
    static gvar3<int> gvnCoarseSubPixIts("Tracker.CoarseSubPixIts",
            TRACKER_COARSE_SUBPIXELITS_DEFAULT, SILENT); // Max sub-pixel iterations for coarse features
    static gvar3<int> gvnCoarseDisabled("Tracker.DisableCoarse",
            TRACKER_COARSE_DISABLE_DEFAULT, SILENT); // Set this to 1 to disable coarse stage (except after recovery)
    static gvar3<double> gvdCoarseMinVel("Tracker.CoarseMinVelocity",
            TRACKER_COARSE_MIN_VELOCITY_DEFAULT, SILENT); // Speed above which coarse stage is used.
    ...
    if (bTryCoarse
            && avPVS[LEVELS - 1].size() + avPVS[LEVELS - 2].size()
                    > *gvnCoarseMin)
    {
        ...
        unsigned int nFound = SearchForPoints(vNextToSearch, nCoarseRange,
                *gvnCoarseSubPixIts);
        ...
        if (nFound >= *gvnCoarseMin) // Were enough found to do any meaningful optimisation?
        {
            ...
            for (int iter = 0; iter < 10; iter++) // If so: do ten Gauss-Newton pose updates iterations.
            {
                ...
                { // Re-project the points on all but the first iteration.
                    for (unsigned int i = 0; i < vIterationSet.size(); i++)
                        if (vIterationSet[i]->bFound)
                            vIterationSet[i]->ProjectAndDerivs(mse3CamFromWorld,
                                    mCamera);
                }
                for (unsigned int i = 0; i < vIterationSet.size(); i++)
                    if (vIterationSet[i]->bFound)
                ...
                Vector<6> v6Update = CalcPoseUpdate(vIterationSet,
                        dOverrideSigma);
    ...
    static gvar3<int> gvnMaxPatchesPerFrame("Tracker.MaxPatchesPerFrame",
            TRACKER_MAX_PATCHES_PER_FRAME_DEFAULT, SILENT);
    ...
    /*
     ...
     glLoadIdentity();
     glOrtho(0,(double)640, (double) 480, 0, -1.0, 1.0);

     glRasterPos2d(0,0);
     glPixelZoom(1,-1);

     unsigned int* mask = new unsigned int[640*480];

     // set to black
     for(int i=0;i<640*480;i++)
     mask[i] = 0x000000FF;

     // set features and region to invisible
     for(vector<TrackerData*>::reverse_iterator it = vIterationSet.rbegin();
     ...
     int xPos = (*it)->v2Image[0]*2;
     int yPos = (*it)->v2Image[1]*2;
     int radius = 6*pow(2.0f,(float)(*it)->nSearchLevel);

     for(int x=xPos-radius;x<=xPos+radius;x++)
     for(int y=yPos-radius;y<=yPos+radius;y++)
     if(x+640*y < 640*480 && x+640*y >= 0)
     mask[x+640*y] = 0x00000000;
     }


     glDrawPixels(640,480,GL_RGBA,GL_UNSIGNED_INT_8_8_8_8,mask);
     delete mask;
     */
    ...
    // Finally, find the mean scene depth from tracked features
    {
        ...
        mappoints_pub.publish(msg);
    }
}

// Find points in the image. Uses the PatchFiner struct stored in TrackerData
int Tracker::SearchForPoints(vector<TrackerData*> &vTD, int nRange,
        int nSubPixIts)
{
    int nFound = 0;
    for (unsigned int i = 0; i < vTD.size(); i++) // for each point..
    {
        ...
        bool bFound = Finder.FindPatchCoarse(ir(TD.v2Image), mCurrentKF,
                nRange);
        TD.bSearched = true;
        if (!bFound)
        {
        ...
        if (nSubPixIts > 0)
        {
            ...
            bool bSubPixConverges = Finder.IterateSubPixToConvergence(
                    mCurrentKF, nSubPixIts);
            if (!bSubPixConverges)
        ...
        double dOverrideSigma, bool bMarkOutliers)
{
    ...
    int nEstimator = 0;
    static gvar3<string> gvsEstimator("TrackerMEstimator",
            TRACKER_M_ESTIMATOR_DEFAULT, SILENT);
    ...
    for (unsigned int f = 0; f < vTD.size(); f++)
    {
        ...
        vdErrorSquared.push_back(
                (double) (TD.v2Error_CovScaled * TD.v2Error_CovScaled));
    };
    ...
    for (unsigned int f = 0; f < vTD.size(); f++)
    {
        ...
        wls.add_mJ(v2[0], (Vector<6> ) (TD.dSqrtInvNoise * m26Jac[0]), dWeight); // These two lines are currently
        wls.add_mJ(v2[1], (Vector<6> ) (TD.dSqrtInvNoise * m26Jac[1]), dWeight); // the slowest bit of poseits
    }
...
void Tracker::ApplyMotionModel()
{
    ...
    if (false && gTransformsGood)
    {
        mse3CamFromWorld = predictedCFromW;
    }
    else
    {
        ...
        Vector<3> diff = mse3CamFromWorld.get_translation()
                - predictedCFromW.get_translation();
        for (int i = 0; i < 3; i++)
            diff[i] = abs(diff[i]);

        if (diff[0] < 0.1 && diff[1] < 0.1 && diff[2] < 0.1)
        {
            gTransformsGood = 1;
            std::cerr << "gTransformsGood toggled" << std::endl;
        }
...
void Tracker::UpdateMotionModel()
{
    ...
    mdVelocityMagnitude = sqrt(
            (double) (mv6CameraVelocity * mv6CameraVelocity));
    ...
    mdMSDScaledVelocityMagnitude = sqrt((double) (v6 * v6));
}
...
void Tracker::AddNewKeyFrame()
{
    ...
    mnLastKeyFrameDroppedClock = clock();
}
...
void Tracker::AssessTrackingQuality()
{
    ...
    else
    {
        ...
        static gvar3<double> gvdQualityGood("Tracker.TrackingQualityGood",
                TRACKER_QUALITY_GOOD_DEFAULT, SILENT);
        static gvar3<double> gvdQualityLost("Tracker.TrackingQualityLost",
                TRACKER_QUALITY_LOST_DEFAULT, SILENT);

        // if(dTotalFracFound > 0.3 /*|| (nTotalAttempted > 200 && nTotalFound > 50)*/)
        if (dTotalFracFound > 0) // Devesh
            mTrackingQuality = GOOD;
        else if (dLargeFracFound < *gvdQualityLost)
        {
            mTrackingQuality = BAD;
            std::cout << "quality lost value " << *gvdQualityGood << std::endl;
        }
        else
    ...
    numPointsFound = nTotalFound;
    numPointsAttempted = nTotalAttempted;
}
