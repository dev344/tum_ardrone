...
void Tracker::Reset() {
	...
	mnLostFrames = 0;
	// [ziquan] mdMSDScaledVelocityMagnitude = 0;
	...
	mnLastKeyFrameDroppedClock = 0;
	mnFrame = 0;
	// [ziquan] mv6CameraVelocity = Zeros;
	// [ziquan] mbJustRecoveredSoUseCoarse = false;
	predictedCFromW = SE3<>();
...
void Tracker::TrackFrame(Image<byte> &imFrame, bool bDraw) {
	...
	if (mMap.IsGood()) {
		if (mnLostFrames < 3)  // .. but only if we're not lost!
			...
			// [ziquan] ApplyMotionModel();
			TrackMap();
			// [ziquan] UpdateMotionModel();
...
void Tracker::tryToRecover() {
	if (mMap.IsGood()) {
		...
		if (true) { // [ziquan] AttemptRecovery()) {
			TrackMap();
			AssessTrackingQuality();
...
void Tracker::TrailTracking_Start() {
	...
	for (unsigned int i = 0; i < vCornersAndSTScores.size() && nToAdd > 0;
			i++) {
...
void Tracker::TrackMap() {
	...
	/* [ziquan] always want to do a coarse stage
