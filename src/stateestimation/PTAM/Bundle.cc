        ...
        mCamera(TCam)
{
    mnCamsToUpdate = 0;
    mnStartRow = 0;
    GV3::Register(mgvnMaxIterations, "Bundle.MaxIterations",
            BUNDLE_MAX_ITERATIONS, SILENT);
    GV3::Register(mgvdUpdateConvergenceLimit,
            "Bundle.UpdateSquaredConvergenceLimit",
            BUNDLE_UPDATE_SQUARED_CONV_LIMIT, SILENT);
...
void Bundle::AddMeas(int nCam, int nPoint, Vector<2> v2Pos,
...
int Bundle::Compute(bool *pbAbortSignal)
{
    ...
    static gvar3<string> gvsMEstimator("BundleMEstimator", BUNDLE_M_ESTIMATOR,
            SILENT);
...
bool Bundle::Do_LM_Step(bool *pbAbortSignal)
{
    ...
    static gvar3<double> gvdMinSigma("Bundle.MinTukeySigma",
            BUNDLE_MIN_TUKEY_SIGMA, SILENT);
            ...
            itr++)
    {
        ...
        double dWeight = MEstimator::SquareRootWeight(meas.dErrorSquared,
                mdSigmaSquared);
        // Re-weight error:
        ...
        dCurrentError += MEstimator::ObjectiveScore(meas.dErrorSquared,
                mdSigmaSquared);
        ...
        else
        {
            for (int m = 0; m < 6; m++)
            {
                ...
                v2CamFrameMotion[0] = (v4Motion[0]
                        - v4Cam[0] * v4Motion[2] * dOneOverCameraZ)
                        * dOneOverCameraZ;
                v2CamFrameMotion[1] = (v4Motion[1]
                        - v4Cam[1] * v4Motion[2] * dOneOverCameraZ)
                        * dOneOverCameraZ;
                meas.m26A.T()[m] = meas.dSqrtInvNoise * m2CamDerivs
                        * v2CamFrameMotion;
            };
        }
        ...
        for (int m = 0; m < 3; m++)
        {
            const Vector<3> v3Motion =
                    cam.se3CfW.get_rotation().get_matrix().T()[m];
            Vector<2> v2CamFrameMotion;
            v2CamFrameMotion[0] = (v3Motion[0]
                    - v4Cam[0] * v3Motion[2] * dOneOverCameraZ)
                    * dOneOverCameraZ;
            v2CamFrameMotion[1] = (v3Motion[1]
                    - v4Cam[1] * v3Motion[2] * dOneOverCameraZ)
                    * dOneOverCameraZ;
            meas.m23B.T()[m] = meas.dSqrtInvNoise * m2CamDerivs
                    * v2CamFrameMotion;
        };
            ...
            && !*pbAbortSignal)
    {
        ...
        for (vector<Point>::iterator itr = mvPoints.begin();
                itr != mvPoints.end(); itr++)
        {
            Point &point = *itr;
        ...
        for (unsigned int j = 0; j < mvCameras.size(); j++)
        {
            ...
            for (unsigned int i = 0; i < mvPoints.size(); i++)
            {
                ...
                v6 -= pMeas->m63W
                        * (mvPoints[i].m3VStarInv * mvPoints[i].v3EpsilonB);
            }
...
#ifndef WIN32
                mS.slice(nJRow, nKRow, 6, 6) -= m63_MIJW_times_m3VStarInv
                        * pMeas_ik->m63W.T();
#else
        ...
        for (unsigned int i = 0; i < mvPoints.size(); i++)
        {
            ...
            for (unsigned int j = 0; j < mvCameras.size(); j++)
            {
                ...
                if (pMeas == NULL || pMeas->bBad)
                    continue;
                v3Sum += pMeas->m63W.T()
                        * vCamerasUpdate.slice(cam.nStartRow, 6);
            }
            Vector<3> v3 = mvPoints[i].v3EpsilonB - v3Sum;
            vMapUpdates.slice(i * 3, 3) = mvPoints[i].m3VStarInv * v3;
            if (isnan(
                    vMapUpdates.slice(i * 3, 3) * vMapUpdates.slice(i * 3, 3)))
            {
                // [Devesh] Commenting out these two nanneries.
                // cerr << "NANNERY! " << endl;
        ...
        double dSumSquaredUpdate = vCamerasUpdate * vCamerasUpdate
                + vMapUpdates * vMapUpdates;
        ...
        for (unsigned int j = 0; j < mvCameras.size(); j++)
        {
            if (mvCameras[j].bFixed)
                ...
                mvCameras[j].se3CfWNew = SE3<>::exp(
                        vCamerasUpdate.slice(mvCameras[j].nStartRow, 6))
                        * mvCameras[j].se3CfW;
        }
        for (unsigned int i = 0; i < mvPoints.size(); i++)
        ...
        cout << setprecision(1) << "L" << mdLambda << setprecision(3)
                << "\tOld " << dCurrentError << "  New " << dNewError
...
double Bundle::FindNewError()
{
            ...
            itr++)
    {
        Meas &meas = *itr;
        // Project the point.
        Vector<3> v3Cam = mvCameras[meas.c].se3CfWNew
                * mvPoints[meas.p].v3PosNew;
        if (v3Cam[2] <= 0)
        {
            dNewError += 1.0;
            cout << ".";
...
void Bundle::GenerateMeasLUTs()
{
    ...
    for (list<Meas>::iterator it = mMeasList.begin(); it != mMeasList.end();
            it++)
...
void Bundle::GenerateOffDiagScripts()
{
    for (unsigned int i = 0; i < mvPoints.size(); i++)
    {
        ...
        for (set<int>::iterator it_j = p.sCameras.begin();
                it_j != p.sCameras.end(); it_j++)
        {
            int j = *it_j;
            if (mvCameras[j].bFixed)
                continue;
            ...
            for (set<int>::iterator it_k = p.sCameras.begin(); it_k != it_j;
                    it_k++)
            {
                int k = *it_k;
