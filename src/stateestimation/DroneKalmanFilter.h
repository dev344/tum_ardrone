...
public:
	...
	inline ScaleStruct(TooN::Vector<3> ptamDist, TooN::Vector<3> imuDist) {
		ptam = ptamDist;
		imu = imuDist;
		pp = ptam * ptam; // ptam[0] * ptam[0] + ptam[1] * ptam[1] + ptam[2] * ptam[2];
		ii = imu * imu; // imu[0] * imu[0] + imu[1] * imu[1] + imu[2] * imu[2];
		pi = imu * ptam; // imu[0] * ptam[0] + imu[1] * ptam[1] + imu[2] * ptam[2];
...
public:
	...
	inline void predict(double ms, double accelerationVar,
			TooN::Vector<2> controlGains = TooN::makeVector(0, 0),
			double coVarFac = 1, double speedVarFac = 1) {
		/* MATLAB:
		 G = [1 ms/1000; 0 1];
	...
	inline void predict(double ms, TooN::Vector<3> vars,
			TooN::Vector<2> controlGains = TooN::makeVector(0, 0)) {
		/* MATLAB:
		 G = [1 ms/1000; 0 1];
		 E = [((ms/1000)^2)/2; ms/1000]; % assume normal distributed constant ACCELERATION.
...
private:
	...
	void predictInternal(geometry_msgs::Twist activeControlInfo,
			int timeSpanMicros, bool useControlGains = true);
...
public:
	...
	void predictUpTo(int timestamp, bool consume = true, bool useControlGains =
			true);
	...
	void updateScaleXYZ(TooN::Vector<3> ptamDiff, TooN::Vector<3> imuDiff,
	...
	inline int getNumGoodPTAMObservations() {
	...
	tum_ardrone::filter_state getPoseAt(ros::Time t,
