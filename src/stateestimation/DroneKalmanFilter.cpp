...
void DroneKalmanFilter::setPing(unsigned int navPing, unsigned int vidPing)
{
    ...
    int new_delayVideo = base_delayVideo + vidPing / (int) 2
            - navPing / (int) 2;
    int new_delayControl = base_delayControl + navPing;

    delayXYZ = std::min(500,
            std::max(40, std::min(new_delayVideo, new_delayXYZ)));
    delayVideo = std::min(500, std::max(40, new_delayVideo));
    delayControl = std::min(200, std::max(50, new_delayControl));

    std::cout << "new delasXYZ: " << delayXYZ << ", delayVideo: " << delayVideo
            << ", delayControl: " << delayControl << std::endl;
}
...
void DroneKalmanFilter::clearPTAM()
{
    ...
    roll_offset = pitch_offset = yaw_offset = x_offset = y_offset = z_offset =
            0;
...
void DroneKalmanFilter::predictInternal(geometry_msgs::Twist activeControlInfo,
        int timeSpanMicros, bool useControlGains)
{
    if (timeSpanMicros <= 0)
        return;
    ...
    bool controlValid = !(activeControlInfo.linear.z > 1.01
            || activeControlInfo.linear.z < -1.01
            || activeControlInfo.linear.x > 1.01
            || activeControlInfo.linear.x < -1.01
            || activeControlInfo.linear.y > 1.01
            || activeControlInfo.linear.y < -1.01
            || activeControlInfo.angular.z > 1.01
            || activeControlInfo.angular.z < -1.01);
    ...
    float rollControlGain = tsSeconds * c3
            * (c4 * max(-0.5, min(0.5, (double) activeControlInfo.linear.y))
                    - roll.state);
    float pitchControlGain = tsSeconds * c3
            * (c4 * max(-0.5, min(0.5, (double) activeControlInfo.linear.x))
                    - pitch.state);
    float yawSpeedControlGain = tsSeconds * c5
            * (c6 * activeControlInfo.angular.z - yaw.state[1]); // at adaption to ros, this has to be reverted for some reason....
    ...
    double forceX = cos(yawRad) * sin(rollRad) * cos(pitchRad)
            - sin(yawRad) * sin(pitchRad);
    double forceY = -sin(yawRad) * sin(rollRad) * cos(pitchRad)
            - cos(yawRad) * sin(pitchRad);

    double vx_gain = tsSeconds * c1 * (c2 * forceX - x.state[1]);
    double vy_gain = tsSeconds * c1 * (c2 * forceY - y.state[1]);
    double vz_gain = tsSeconds * c7
            * (c8 * activeControlInfo.linear.z
                    * (activeControlInfo.linear.z < 0 ? 2 : 1) - z.state[1]);
    ...
    yaw.predict(tsMillis, varAccelerationError_yaw,
            TooN::makeVector(tsSeconds * yawSpeedControlGain / 2,
                    yawSpeedControlGain), 1, 5 * 5);
    yaw.state[0] = angleFromTo(yaw.state[0], -180, 180);

    x.predict(tsMillis, varAccelerationError_xy,
            TooN::makeVector(tsSeconds * vx_gain / 2, vx_gain), 0.0001);
    y.predict(tsMillis, varAccelerationError_xy,
            TooN::makeVector(tsSeconds * vy_gain / 2, vy_gain), 0.0001);
    z.predict(tsMillis,
            TooN::makeVector(tsSeconds * tsSeconds * tsSeconds * tsSeconds,
                    9 * tsSeconds, tsSeconds * tsSeconds * tsSeconds * 3),
...
void DroneKalmanFilter::observeIMU_XYZ(const ardrone_autonomy::Navdata* nav)
{
    ...
    if (abs(lastX - x.state[0]) > 0.2 && lastX != 0)
    {
        ...
        ROS_WARN(
                "detected large x jump. removing. should not happen usually (only e.g. if no navdata for a long time, or agressive re-scaling)");
        x.state[0] = lastX;
    }
...
//	}
    ...
    if (last_z_IMU != nav->altd || nav->header.seq - last_z_packageID > 8)
    {
        ...
        if (lastPosesValid)
        {
            ...
            double observedHeight = baselineZ_Filter
                    + 0.5 * (imuHeightDiff + last_z_heightDiff);
...
void DroneKalmanFilter::observeIMU_RPY(const ardrone_autonomy::Navdata* nav)
{
    ...
    else if (abs(observedYaw - yaw.state[0]) < 10)
    {
...
void DroneKalmanFilter::sync_rpy(double roll_global, double pitch_global,
        double yaw_global)
{
    if (allSyncLocked)
        return;
    ...
    {
        roll_offset = rp_offset_framesContributed * roll_offset + roll.state
                - roll_global;
        pitch_offset = rp_offset_framesContributed * pitch_offset + pitch.state
                - pitch_global;
    }
...
void DroneKalmanFilter::sync_xyz(double x_global, double y_global,
        double z_global)
{
    if (allSyncLocked)
        return;
...
void DroneKalmanFilter::flushScalePairs()
{
    ...
    for (unsigned int i = 0; i < scalePairs->size(); i++)
        (*fle) << (*scalePairs)[i].ptam[0] << " " << (*scalePairs)[i].ptam[1]
                << " " << (*scalePairs)[i].ptam[2] << " "
                << (*scalePairs)[i].imu[0] << " " << (*scalePairs)[i].imu[1]
                << " " << (*scalePairs)[i].imu[2] << std::endl;
    fle->flush();
    fle->close();
    delete fle;
}
        ...
        TooN::Vector<3> imuDiff, TooN::Vector<3> OrgPtamPose)
{
    ...
    if (s.imuNorm < 0.05 || s.ptamNorm < 0.05)
        return;
    ...
    double median =
            (*scalePairs)[((*scalePairs).size() + 1) / 2].alphaSingleEstimate;
    ...
    {
        if ((*scalePairs).size() < 5
                || ((*scalePairs)[i].alphaSingleEstimate > median * 0.2
                        && (*scalePairs)[i].alphaSingleEstimate < median / 0.2))
        {
            ...
            sumIIxy += (*scalePairs)[i].imu[0] * (*scalePairs)[i].imu[0]
                    + (*scalePairs)[i].imu[1] * (*scalePairs)[i].imu[1];
            sumPPxy += (*scalePairs)[i].ptam[0] * (*scalePairs)[i].ptam[0]
                    + (*scalePairs)[i].ptam[1] * (*scalePairs)[i].ptam[1];
            sumPIxy += (*scalePairs)[i].ptam[0] * (*scalePairs)[i].imu[0]
                    + (*scalePairs)[i].ptam[1] * (*scalePairs)[i].imu[1];
    ...
    double scale_Filtered = (*scalePairs)[0].computeEstimator(sumPP, sumII,
            sumPI, 0.2, 0.01);
    double scale_Unfiltered = (*scalePairs)[0].computeEstimator(
            sumPP + totSumPP, sumII + totSumII, sumPI + totSumPI, 0.2, 0.01);
    double scale_PTAMSmallVar = (*scalePairs)[0].computeEstimator(
            sumPP + totSumPP, sumII + totSumII, sumPI + totSumPI, 0.00001, 1);
    double scale_IMUSmallVar = (*scalePairs)[0].computeEstimator(
            sumPP + totSumPP, sumII + totSumII, sumPI + totSumPI, 1, 0.00001);

    double scale_Filtered_xy = (*scalePairs)[0].computeEstimator(sumPPxy,
            sumIIxy, sumPIxy, 0.2, 0.01);
    double scale_Filtered_z = (*scalePairs)[0].computeEstimator(sumPPz, sumIIz,
            sumPIz, 0.2, 0.01);
    ...
    printf(
            "scale: in: %i; out: %i, filt: %.3f; xyz: %.1f < %.1f < %.1f; xy: %.1f < %.1f < %.1f; z: %.1f < %.1f < %.1f;\n",
            numIn, numOut, scale_Filtered, scale_PTAMSmallVar, scale_Unfiltered,
            scale_IMUSmallVar,
            (*scalePairs)[0].computeEstimator(sumPPxy, sumIIxy, sumPIxy,
                    0.00001, 1), scale_Filtered_xy,
            (*scalePairs)[0].computeEstimator(sumPPxy, sumIIxy, sumPIxy, 1,
                    0.00001),
            (*scalePairs)[0].computeEstimator(sumPPz, sumIIz, sumPIz, 0.00001,
                    1), scale_Filtered_z,
            (*scalePairs)[0].computeEstimator(sumPPz, sumIIz, sumPIz, 1,
                    0.00001));

    if (scale_Filtered > 0.1)
        z_scale = xy_scale = scale_Filtered;
    else
        ROS_WARN("calculated scale is too small %.3f, disallowing!",
                scale_Filtered);
        ...
        bool useControlGains)
{
    ...
    std::deque<geometry_msgs::TwistStamped>::iterator controlIterator =
            velQueue->begin();
    while (controlIterator != velQueue->end()
            && controlIterator + 1 != velQueue->end()
            && getMS((controlIterator + 1)->header.stamp)
                    <= predictdUpToTimestamp - delayControl)
        if (consume)
        {
            velQueue->pop_front();
    ...
    std::deque<ardrone_autonomy::Navdata>::iterator xyzIterator =
            navdataQueue->begin();
    while (xyzIterator != navdataQueue->end()
            && getMS(xyzIterator->header.stamp)
                    <= predictdUpToTimestamp + delayXYZ)
        xyzIterator++;

    std::deque<ardrone_autonomy::Navdata>::iterator rpyIterator =
            navdataQueue->begin();
    while (rpyIterator != navdataQueue->end()
            && getMS(rpyIterator->header.stamp)
                    <= predictdUpToTimestamp + delayRPY)
        if (consume)
        {
    ...
    while (true)
    {
        ...
        predictInternal(
                useControlGains ?
                        controlIterator->twist : geometry_msgs::Twist(),
                (predictTo - predictdUpToTimestamp) * 1000,
                useControlGains
                        && getMS(controlIterator->header.stamp) + 200
                                > predictdUpToTimestamp - delayControl); // control max. 200ms old.
        ...
        if (rpyIterator != navdataQueue->end()
                && getMS(rpyIterator->header.stamp) - delayRPY == predictTo)
        {
            if (this->useNavdata)
        ...
        if (xyzIterator != navdataQueue->end()
                && getMS(xyzIterator->header.stamp) - delayXYZ == predictTo)
        {
            if (this->useNavdata)
        ...
        if (consume)
        {
            if (node->logfileFilter != NULL)
            {
                ...
                (*(node->logfileFilter)) << predictdUpToTimestamp << " " << 0
                        << " " << 0 << " " << 0 << " " << 0 << " " << 0 << " "
                        << 0 << " " << controlIterator->twist.linear.y << " "
                        << controlIterator->twist.linear.x << " "
                        << controlIterator->twist.linear.z << " "
                        << controlIterator->twist.angular.z << " "
                        << (observedRPY ? rpyIterator->rotX : -1) << " "
                        << (observedRPY ? rpyIterator->rotY : -1) << " "
                        << (observedRPY ? lastdYaw : -1) << " "
                        << (observedXYZ ? xyzIterator->vx : -1) << " "
                        << (observedXYZ ? xyzIterator->vy : -1) << " "
                        << (observedXYZ ? lastdZ : -1) << " " << x.state[0]
                        << " " << y.state[0] << " " << z.state[0] << " "
                        << roll.state << " " << pitch.state << " "
                        << yaw.state[0] << " " << x.state[1] << " "
                        << y.state[1] << " " << z.state[1] << " "
                        << yaw.state[1] << " " << lastVXGain << " "
                        << lastVYGain << " " << "\n";
...
TooN::Vector<3> DroneKalmanFilter::transformPTAMObservation(double x, double y,
        double z)
{
...
{
    obs.slice<0, 3>() = transformPTAMObservation(obs[0], obs[1], obs[2],
            obs[5]);
...
{
    return TooN::makeVector(x.state[0], y.state[0], z.state[0], roll.state,
            pitch.state, yaw.state[0]);
}
...
tum_ardrone::filter_state DroneKalmanFilter::getCurrentPoseSpeed()
{
    ...
    if (s.roll * s.roll < 0.001)
        s.roll = 0;
    if (s.pitch * s.pitch < 0.001)
        s.pitch = 0;
    if (s.yaw * s.yaw < 0.001)
        s.yaw = 0;
    if (s.dx * s.dx < 0.001)
        s.dx = 0;
    if (s.dy * s.dy < 0.001)
...
{
    return TooN::makeVector(x.state[0], y.state[0], z.state[0], roll.state,
            pitch.state, yaw.state[0], x.state[1], y.state[1], z.state[1],
            yaw.state[1]);
}

TooN::Vector<10> DroneKalmanFilter::getCurrentPoseSpeedVariances()
{
    return TooN::makeVector(x.var(0, 0), y.var(0, 0), z.var(0, 0), roll.var,
            pitch.var, yaw.var(0, 0), x.var(1, 1), y.var(1, 1), z.var(1, 1),
            yaw.var(1, 1));
}
...
TooN::Vector<6> DroneKalmanFilter::getCurrentOffsets()
{
    ...
    if (rp_offset_framesContributed > 1)
        res.slice<3, 3>() = TooN::makeVector(roll_offset, pitch_offset,
                yaw_offset);
    return res;
}
TooN::Vector<3> DroneKalmanFilter::getCurrentScales()
{
    return TooN::makeVector(scale_xyz_initialized ? xy_scale : 1,
            scale_xyz_initialized ? xy_scale : 1,
            scale_xyz_initialized ? z_scale : 1);
}
TooN::Vector<3> DroneKalmanFilter::getCurrentScalesForLog()
{
    return TooN::makeVector(scale_xyz_initialized ? scale_from_xy : 1,
...
// >>>>>>> master
    ...
    (*scalePairs).push_back(
            ScaleStruct(TooN::makeVector(0.2, 0.2, 0.2) / sqrt(scales[0]),
